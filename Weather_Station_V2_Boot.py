...
rtc.datetime((2024, 11, 29, 21, 7, 47, 0))
...
def log_data():
    try:
        ...
        with open("/sd/weather_data.csv", "a") as file:
            file.write(f"{year}-{month:02d}-{day:02d},{hour:02d}:{minute:02d}:{second:02d},{temp_c1},{temp_f1},{hum1},{temp_c2},{temp_f2},{hum2},{uv_value},{uv_voltage}\n")
    except OSError as e:
...
def display_data():
    try:
        ...
        oled.text(f"Date:{month:02d}-{day:02d}-{year}", 0, 10)
...
# Main loop to read sensors, update the OLED, and log data every 20 minutes
sample_interval = 20 * 60  # 20 minutes in seconds
display_interval = 15  # 15 seconds for display refresh
